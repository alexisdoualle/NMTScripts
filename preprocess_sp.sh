...
slines=$(wc -l $fileName.$sl | cut -f1 -d' ')
tlines=$(wc -l $fileName.$tl | cut -f1 -d' ')
if [ $slines -eq $tlines ]; then
  echo "number of lines match"
else
  echo "number of lines in source and target files don't match"
  exit
fi
...
for arg in "${files[@]}"; do
    echo "$arg"
    head -n -$validationSize $arg.$sl >> $fileName-train.$sl
    head -n -$validationSize $arg.$tl >> $fileName-train.$tl
    tail -n -$validationSize $arg.$sl >> $fileName-valid.$sl
    tail -n -$validationSize $arg.$tl >> $fileName-valid.$tl
done

# head -n -$validationSize $fileName.$sl > $fileName-train.$sl
# head -n -$validationSize $fileName.$tl > $fileName-train.$tl
# tail -n -$validationSize $fileName.$sl > $fileName-valid.$sl
# tail -n -$validationSize $fileName.$tl > $fileName-valid.$tl
...
data:
  ...
  source_words_vocabulary: data/'$fileName-$sl$tl.vocab'
  target_words_vocabulary: data/'$fileName-$sl$tl.vocab'
